#!/usr/bin/env bash
# OS prep for PLAYGROUND — AppArmor Profile Enforcement
#
# Runs once when the environment comes up. Environment preparation only:
# install the AppArmor userspace tooling, create the "appservice" demo
# daemon and its relocated log directory (with correct DAC perms), and
# load a deliberately too-strict AppArmor profile in enforce mode so the
# write to /srv/applogs is genuinely denied and a fresh DENIED audit
# record is always available to look at. There is no task and no grading.
set -eu

echo "[playground] apparmor-mac-enforcement: installing apparmor userspace tools ..."
export DEBIAN_FRONTEND=noninteractive
sudo apt-get update -qq
sudo apt-get install -y -qq apparmor-utils apparmor-profiles
...
# running even if a write attempt fails, so a fresh denial is always
...
# the only log path the currently-installed profile below actually permits.
sudo mkdir -p /var/log/appservice
sudo chown appservice:appservice /var/log/appservice
sudo chmod 750 /var/log/appservice

# The service has ALREADY been reconfigured (by "a colleague") to log
# here instead. DAC ownership/permissions on this directory are already
...
# override file under /etc/apparmor.d/local/ so an admin can extend
# policy without editing the shipped profile directly. Ship it empty.
...
# real DENIED audit entry already exists before anyone logs in.
sleep 8

echo "[playground] ready. Try:  sudo aa-status   and   sudo journalctl -k | grep 'apparmor=\"DENIED\"'"
