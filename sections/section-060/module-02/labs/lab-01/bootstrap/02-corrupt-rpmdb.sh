#!/usr/bin/env bash
# Simulates a real, unattended RPM database corruption inside rpmbox: a
# disk-full/interrupted-write style truncation-plus-overwrite of the actual
# sqlite-backed rpmdb file Rocky Linux 9 uses. RHEL 9 / Rocky 9 default to
# a sqlite backend at /var/lib/rpm/rpmdb.sqlite -- NOT the older Berkeley
# DB Packages/__db.* layout some older tutorials assume. Already-installed
# package files on disk (under /usr, /etc, etc.) are left completely
# untouched -- only the database's own file is corrupted.

set -eu

echo "Installing a few extra real packages inside rpmbox first, so there is realistic data in the RPM database before we corrupt it..."
sudo docker exec rpmbox dnf -y install vim-enhanced wget tree
...
sudo docker exec rpmbox bash -c '
  ...
  # a sqlite database and enough of it remains readable for rpm --rebuilddb
  # to salvage what it can -- this mirrors a real interrupted-write event,
  # not a total wipe.
  offset=$(( size * 40 / 100 ))
...
echo "rpmbox's RPM database is now corrupted. Installed package files are untouched. This is the student's diagnose-and-repair task."
