...
# Rocky Linux 9 container ("rpmbox") that provides a REAL rpm/dnf
# environment for this capstone. This repo's only VM base image is Ubuntu
# 24.04 -- there is no Rocky/RHEL VM available, so this is how we give
# students genuine rpm/dnf tooling against a real RPM database rather than
# a faked one. See section-060's module course.md files and this lab's
