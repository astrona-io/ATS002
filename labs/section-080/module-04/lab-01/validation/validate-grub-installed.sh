#!/usr/bin/env bash
# Confirms GRUB's own installed boot-sector/EFI code was genuinely
# reinstalled during this session -- not just left over from the base VM
# image -- and that a final `grub-install --recheck` confirms it is
# healthy. Runs entirely over the still-live SSH session; no reboot
