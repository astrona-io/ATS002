#!/usr/bin/env bash
# Bootstrap: serves /srv/vendor-repo over plain HTTP on 127.0.0.1:8100 as a
# persistent systemd unit, standing in for the vendor's real repository
...
[Service]
ExecStart=/usr/bin/python3 -m http.server 8100 --directory /srv/vendor-repo --bind 127.0.0.1
Restart=always
User=root
...
for _ in $(seq 1 10); do
  if curl -fsS "http://127.0.0.1:8100/vendor-nginx-archive-keyring.asc" >/dev/null 2>&1; then
    break
  fi
  sleep 1
done
